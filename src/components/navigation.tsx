...
export function Navigation() {
  ...
  return (
    <nav className="bg-white shadow-lg sticky top-0 z-50" style={{ pointerEvents: 'auto' }}>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="text-gray-700 hover:text-orange-500 transition-colors duration-200 font-medium cursor-pointer"
                onClick={(e) => {
                  e.preventDefault()
            ...
            <Button asChild className="bg-gradient-to-r from-yellow-400 to-orange-500 hover:from-yellow-500 hover:to-orange-600 text-white font-semibold">
              <Link 
                href="/activities?published=true&upcoming=true"
                prefetch={true}
                onClick={(e) => {
                  e.stopPropagation()
                }}
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-white border-t">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="block px-3 py-2 text-gray-700 hover:text-orange-500 hover:bg-gray-50 rounded-md transition-colors duration-200 cursor-pointer"
                  prefetch={true}
                  onClick={(e) => {
                    setIsOpen(false)
                    e.stopPropagation()
